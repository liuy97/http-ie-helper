...
const config = {
  libraryName: 'http-ie-helper',
  unscopedLibraryName: 'http-ie-helper',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
const readyToRelease = () => {
  ...
  let canGhRelease = true; // argv.ghToken || process.env.CONVENTIONAL_GITHUB_RELEASER_TOKEN;
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /http-ie-helper/ --deploy-url /http-ie-helper/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run http-ie-helper-demo:server`, { cwd: `${config.demoDir}` }))
